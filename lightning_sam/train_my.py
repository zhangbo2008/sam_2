...
#============定制自己的数据集.
    ...
    config = {
        "num_devices": 'auto',
        "batch_size": 2,
        "num_workers": 0, #表示只使用主进程来读取.windows都用0.linux可以改.
        "num_epochs": 20,
        "eval_interval": 2,
        ...
        "model": {
            "type": 'vit_b',
            "checkpoint": "/mnt/e/sam_vit_b_01ec64.pth",
            "freeze": {
                "image_encoder": True,
                "prompt_encoder": True,
                "mask_decoder": False,
        ...
        "dataset": {
            "train": {
              "root_dir": os.path.join(ppp,"val2017"),
                "annotation_file": os.path.join(ppp,"instances_val2017.json")
            },#========cocoshujuji:https://blog.csdn.net/W1995S/article/details/113123127  单个图片数据集可以在json文件里面下.#==========下载好2017的训练数据. 然后进行训练. 这里面train和val都配置一样的.
             "val": {
                "root_dir": os.path.join(ppp,"val2017"),
                "annotation_file": os.path.join(ppp,"instances_val2017.json")
            },
